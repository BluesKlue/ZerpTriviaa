...
function optionSelected(answer) {
    ...
    if (userAnswer == correctAnswer) {
        ...
        //if answer is incorrect, auto select the correct option
...
function questionCounter(index) {
    const questionToal = document.querySelector('.question-total');
    questionToal.textContent = `${index} of ${questions.length} Questions`;
}
...
function showResultBox() {
    ...
    let progress = setInterval(() => {
        ...
        circularProgress.style.background = `conic-gradient(#ff4500 ${progressStartValue * 3.6}deg, rgba(225, 225, 225, .1) 0deg)`;
